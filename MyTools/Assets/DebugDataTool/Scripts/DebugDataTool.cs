...
public class DebugDataTool : EditorWindow
{
    ...
    private Rect dropDownButtonRect = new Rect(130, 94, 100, 20);
    ...
    static DebugDataTool window = null;
    #endregion Variables
    ...
    private void OnGUI()
    {
        ...
        if (debugHolders.Count != 0)
        {
            EditorGUI.LabelField(dropDownLabelRect, "Choose data to look at, than press the button to view.");
            dropDownIndex = EditorGUI.Popup(dropDownMenuRect, dropDownIndex, dataNames.ToArray());
            if (GUI.Button(dropDownButtonRect, "Select Data"))
            {
                SelectData();
            }
    ...
    {
        currentDebugHolder = debugHolders[dropDownIndex];
    }
    #endregion GetData
